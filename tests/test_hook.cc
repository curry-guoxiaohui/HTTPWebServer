 ...
 */
void test_sleep() {
    GXH_LOG_INFO(g_logger) << "test_sleep begin";
    gxh::IOManager iom;
    ...
    iom.schedule([] {
        sleep(2);
        GXH_LOG_INFO(g_logger) << "sleep 2";
    });

    iom.schedule([] {
        sleep(3);
        GXH_LOG_INFO(g_logger) << "sleep 3";
    });

    GXH_LOG_INFO(g_logger) << "test_sleep end";
}
...
void test_sock() {
    ...
    GXH_LOG_INFO(g_logger) << "begin connect";
    int rt = connect(sock, (const sockaddr*)&addr, sizeof(addr));
    GXH_LOG_INFO(g_logger) << "connect rt=" << rt << " errno=" << errno;
    ...
    GXH_LOG_INFO(g_logger) << "send rt=" << rt << " errno=" << errno;
    ...
    GXH_LOG_INFO(g_logger) << "recv rt=" << rt << " errno=" << errno;
