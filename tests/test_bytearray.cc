...
#define XX(type, len, write_fun, read_fun, base_len)               \
    {                                                              \
        ...
        gxh::ByteArray::ptr ba(new gxh::ByteArray(base_len));  \
        for (auto &i : vec) {                                      \
            ba->write_fun(i);                                      \
        }                                                          \
        ba->setPosition(0);                                        \
        ...
        GXH_ASSERT(ba->getReadSize() == 0);                      \
        GXH_LOG_INFO(g_logger) << #write_fun "/" #read_fun       \
                                               " (" #type ") len=" \
                                 << len                            \
                                 << " base_len=" << base_len       \
                                 << " size=" << ba->getSize();     \
...
#define XX(type, len, write_fun, read_fun, base_len)                                  \
    {                                                                                 \
        ...
        GXH_LOG_INFO(g_logger) << #write_fun "/" #read_fun                          \
                                 ...
                                 << " size=" << ba->getSize();                        \
        ba->setPosition(0);                                                           \
        GXH_ASSERT(ba->writeToFile("/tmp/" #type "_" #len "-" #read_fun ".dat"));   \
        gxh::ByteArray::ptr ba2(new gxh::ByteArray(base_len * 2));                \
        GXH_ASSERT(ba2->readFromFile("/tmp/" #type "_" #len "-" #read_fun ".dat")); \
        ba2->setPosition(0);                                                          \
        GXH_ASSERT(ba->toString() == ba2->toString());                              \
        GXH_ASSERT(ba->getPosition() == 0);                                         \
        GXH_ASSERT(ba2->getPosition() == 0);                                        \
    }
    XX(int8_t, 100, writeFint8, readFint8, 1);
    XX(uint8_t, 100, writeFuint8, readFuint8, 1);
    XX(int16_t, 100, writeFint16, readFint16, 1);
