...
gxh::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = 
    gxh::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine> &old_value, const std::set<LogDefine> &new_value){
            GXH_LOG_INFO(GXH_LOG_ROOT()) << "on log config changed";
            for(auto &i : new_value) {
                auto it = old_value.find(i);
                gxh::Logger::ptr logger;
                if(it == old_value.end()) {
                    // 新增logger
                    logger = GXH_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        // 修改的logger
                        logger == GXH_LOG_NAME(i.name);
                ...
                for(auto &a : i.appenders) {
                    gxh::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        // 如果以daemon方式运行，则不需要创建终端appender
                        if(!gxh::EnvMgr::GetInstance()->has("d")) {
            ...
            for(auto &i : old_value) {
                ...
                if(it == new_value.end()) {
                    auto logger = GXH_LOG_NAME(i.name);
