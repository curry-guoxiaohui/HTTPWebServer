...
HttpConnection::~HttpConnection() {
    GXH_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    //         GXH_LOG_INFO(g_logger) << "content_len=" << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = gxh::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            GXH_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            GXH_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            GXH_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= gxh::GetCurrentMS())
