...
static gxh::Logger::ptr g_logger = GXH_LOG_NAME("http");

static gxh::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    gxh::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

static gxh::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    gxh::Config::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

static gxh::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    gxh::Config::Lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

static gxh::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    gxh::Config::Lookup("http.response.max_body_size", (uint64_t)(64 * 1024 * 1024), "http response max body size");
...
static int on_request_message_begin_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_request_message_begin_cb";
    return 0;
}
...
static int on_request_headers_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_request_headers_complete_cb";
...
static int on_request_message_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_request_message_complete_cb";
    HttpRequestParser *parser = static_cast<HttpRequestParser *>(p->data);
...
static int on_request_chunk_header_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_request_chunk_header_cb";
    return 0;
}
...
static int on_request_chunk_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_request_chunk_complete_cb";
    return 0;
}
...
static int on_request_url_cb(http_parser *p, const char *buf, size_t len) {
    GXH_LOG_DEBUG(g_logger) << "on_request_url_cb, url is:" << std::string(buf, len);
    ...
    if (ret != 0) {
        GXH_LOG_DEBUG(g_logger) << "parse url fail";
        return 1;
    }
...
static int on_request_header_field_cb(http_parser *p, const char *buf, size_t len) {
    ...
    GXH_LOG_DEBUG(g_logger) << "on_request_header_field_cb, field is:" << field;
    HttpRequestParser *parser = static_cast<HttpRequestParser *>(p->data);
...
static int on_request_header_value_cb(http_parser *p, const char *buf, size_t len) {
    ...
    GXH_LOG_DEBUG(g_logger) << "on_request_header_value_cb, value is:" << value;
    HttpRequestParser *parser = static_cast<HttpRequestParser *>(p->data);
...
static int on_request_status_cb(http_parser *p, const char *buf, size_t len) {
    GXH_LOG_DEBUG(g_logger) << "on_request_status_cb, should not happen";
    return 0;
}
...
static int on_request_body_cb(http_parser *p, const char *buf, size_t len) {
    ...
    GXH_LOG_DEBUG(g_logger) << "on_request_body_cb, body is:" << body;
    HttpRequestParser *parser = static_cast<HttpRequestParser *>(p->data);
...
size_t HttpRequestParser::execute(char *data, size_t len) {
    ...
    if (m_parser.upgrade) {
        //处理新协议，暂时不处理
        GXH_LOG_DEBUG(g_logger) << "found upgrade, ignore";
        setError(HPE_UNKNOWN);
    } else if (m_parser.http_errno != 0) {
        GXH_LOG_DEBUG(g_logger) << "parse request fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
...
static int on_response_message_begin_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_response_message_begin_cb";
    return 0;
}
...
static int on_response_headers_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_response_headers_complete_cb";
...
static int on_response_message_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_response_message_complete_cb";
    HttpResponseParser *parser = static_cast<HttpResponseParser *>(p->data);
    parser->setFinished(true);
    return 0;
}
...
static int on_response_chunk_header_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_response_chunk_header_cb";
    return 0;
}
...
static int on_response_chunk_complete_cb(http_parser *p) {
    GXH_LOG_DEBUG(g_logger) << "on_response_chunk_complete_cb";
    return 0;
}
...
static int on_response_url_cb(http_parser *p, const char *buf, size_t len) {
    GXH_LOG_DEBUG(g_logger) << "on_response_url_cb, should not happen";
    return 0;
}
...
static int on_response_header_field_cb(http_parser *p, const char *buf, size_t len) {
    std::string field(buf, len);
    GXH_LOG_DEBUG(g_logger) << "on_response_header_field_cb, field is:" << field;
    HttpResponseParser *parser = static_cast<HttpResponseParser *>(p->data);
    parser->setField(field);
    return 0;
}
...
static int on_response_header_value_cb(http_parser *p, const char *buf, size_t len) {
    std::string value(buf, len);
    GXH_LOG_DEBUG(g_logger) << "on_response_header_value_cb, value is:" << value;
    HttpResponseParser *parser = static_cast<HttpResponseParser *>(p->data);
    parser->getData()->setHeader(parser->getField(), value);
    return 0;
}
...
static int on_response_status_cb(http_parser *p, const char *buf, size_t len) {
    GXH_LOG_DEBUG(g_logger) << "on_response_status_cb, status code is: " << p->status_code << ", status msg is: " << std::string(buf, len);
    HttpResponseParser *parser = static_cast<HttpResponseParser *>(p->data);
    parser->getData()->setStatus(HttpStatus(p->status_code));
    return 0;
}
...
static int on_response_body_cb(http_parser *p, const char *buf, size_t len) {
    std::string body(buf, len);
    GXH_LOG_DEBUG(g_logger) << "on_response_body_cb, body is:" << body;
    HttpResponseParser *parser = static_cast<HttpResponseParser *>(p->data);
    parser->getData()->appendBody(body);
    return 0;
}
...
size_t HttpResponseParser::execute(char *data, size_t len) {
    ...
    if (m_parser.http_errno != 0) {
        GXH_LOG_DEBUG(g_logger) << "parse response fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
        setError((int8_t)m_parser.http_errno);
    } else {
        if (nparsed < len) {
            memmove(data, data + nparsed, (len - nparsed));
