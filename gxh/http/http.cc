    ...
    do {                                                                                                   \
        ...
        if (0) {                                                                                           \
            std::cout << "<key>:" << str.substr(last, key - last)                                          \
                      << " <decoded>:" << gxh::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                 \
                      << " <decoded>:" << gxh::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                        \
        }                                                                                                  \
                                                                                                           \
        m.insert(std::make_pair(gxh::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                gxh::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos) {                                                                    \
            break;                                                                                         \
        }                                                                                                  \
        ++pos;                                                                                             \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', gxh::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
...
void HttpResponse::setCookie(const std::string &key, const std::string &val,
    ...
    ss << key << "=" << val;
    if (expired > 0) {
        ss << ";expires=" << gxh::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if (!domain.empty()) {
        ss << ";domain=" << domain;
    }
