...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (GXH_UNLIKELY(fd_ctx->events & event)) {
        GXH_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                                  << " event=" << (EPOLL_EVENTS)event
                                  << " fd_ctx.event=" << (EPOLL_EVENTS)fd_ctx->events;
        GXH_ASSERT(!(fd_ctx->events & event));
    }
    ...
    if (rt) {
        GXH_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
    ...
    GXH_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        GXH_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
    }
    return 0;
}
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        if( GXH_UNLIKELY(stopping(next_timeout))) {
            GXH_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
            break;
        }
