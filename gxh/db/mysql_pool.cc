...
static gxh::ConfigVar<MysqlDefine>::ptr g_mysql_define
         = gxh::Config::Lookup("db", MysqlDefine(), "mysql config");
...
struct MysqlInit {
    MysqlInit() {
        g_mysql_define->addListener([](const MysqlDefine& old_value, const MysqlDefine& new_value) {
            ...
            GXH_LOG_INFO(g_logger) << "Mysql config have been changed...";
        });
        // GXH_LOG_INFO(g_logger) << "Mysqlinit";
        // GXH_LOG_INFO(g_logger) << g_mysql_define->toString();
    }
};
...
void Mysql_pool::init(std::string url, std::string User, std::string PassWord, std::string DataBaseName
	...
	for (int i = 0; i < MaxConn; i++)
	{
		...
		if (con == NULL)
		{
			GXH_LOG_ERROR(g_logger) << "MySQL Error: mysql_init";
			exit(1);
		}
		...
		if (con == NULL)
		{
			GXH_LOG_ERROR(g_logger) << "MySQL Error: mysql_real_connect";
			exit(1);
		}
...
bool Mysql_pool::Query(const char* ch, std::vector<std::vector<std::string>>& out) {
    ...
    con = GetConnection();
    if(!con){
        GXH_LOG_ERROR(g_logger) << "MySQL Error: no connection";
        return false;
    }
    
    if(mysql_query(con, ch)) {
        GXH_LOG_ERROR(g_logger) << "MySQL Error: mysql_query" << "----error: " << mysql_error(con);
        return false;
    }
    ...
    // GXH_LOG_INFO(g_logger) << " mysql_query call...";
