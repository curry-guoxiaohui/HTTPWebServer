#ifndef __DB_MYSQL_POOL__
#define __DB_MYSQL_POOL__
...
#define GXH_MYSQL_ROOT() gxh::MysqlMgr::GetInstance()
...
typedef gxh::Singleton<Mysql_pool> MysqlMgr;
/**
 * @brief: 数据库定义格式
 */
struct MysqlDefine {
    std::string url = "192.168.138.128";
...
public:
    ...
    #define XX(var, type) \
        ...
        XX(url, std::string);
        XX(user, std::string);
        XX(password, std::string);
        XX(databasename, std::string);
        XX(port, int);
        XX(maxconn, int);
    #undef XX  
        return md;
