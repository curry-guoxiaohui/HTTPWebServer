...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host,
    ...
    if (error) {
        GXH_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    while (next) {
        ...
        GXH_LOG_DEBUG(g_logger) << "family:" << next->ai_family << ", sock type:" << next->ai_socktype;
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result,
    ...
    if (getifaddrs(&results) != 0) {
        GXH_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                     " err="
    ...
    try {
        ...
        GXH_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
    ...
    if (error) {
        GXH_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                                  << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
    ...
    if (result <= 0) {
        GXH_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
    ...
    if (result <= 0) {
        GXH_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                                  << port << ") rt=" << result << " errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
